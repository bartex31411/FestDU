...
NDefines.NMilitary.WITHDRAWING_SPEED_FACTOR = 0.35               -- speed bonus when withdrawing
...
NDefines.NGame.GAME_SPEED_SECONDS = { 10, 0.3, 0.2, 0.04, 0.0 } -- 0.375, 0.175, 0.1, 0.035, 0.0 game speeds for each level. Must be 5 entries with last one 0 for unbound
---------------------------------------------------------------
...
NDefines.NTechnology.BASE_YEAR_AHEAD_PENALTY_FACTOR = 3.1 -- Vanilla is 2.0, means 300% increase in research time per year ahead
...
NDefines.NMilitary.ARMOR_VS_AVERAGE = 0.17 -- how to weight in highest armor & pen vs the division average
...
NDefines.NMilitary.LAND_AIR_COMBAT_ORG_DAMAGE_MODIFIER =	0.0125 	--global damage modifier 
...
NDefines.NAir.ANTI_AIR_MAXIMUM_DAMAGE_REDUCTION_FACTOR = 0.77 -- (.75) Maximum damage reduction factor applied to incoming air attacks against units with AA.
...
NDefines.NAir.ESCORT_FACTOR = 2 -- (2 -> 3) to make sure that escorted planes are still capable of bombing, with equal escorts/interceptors most of bombers get through Keep in mind that these values will also affect how cas/tac/strat bombers work, they make escorting planes much more important (which imo is 100% fine)

NDefines.NAir.DISRUPTION_DEFENCE_SPEED_FACTOR = 75
...
NDefines.NMilitary.RIVER_CROSSING_PENALTY = -0.20
NDefines.NMilitary.RIVER_CROSSING_PENALTY_LARGE = -0.30
...
NDefines.NAir.COMBAT_DAMAGE_SCALE = 0.1-- (0.1)
...
NDefines.NAir.COMBAT_MULTIPLANE_CAP = 1 -- (3.0) How many of our planes can engage per enemy plane, reduced to encourage trading
NDefines.NAir.AIR_COMBAT_FINAL_DAMAGE_SCALE = 0.015  -- 0.015	 % how many max disrupted only planes are allowed to die in a single combat

NDefines.NAir.DISRUPTION_DETECTION_FACTOR = 1
...
---------------------------------------------------------------
NDefines.NAir.EFFICIENCY_REGION_CHANGE_DAILY_GAIN_TACTICAL_BOMBER = 0.132	-- vanilla 0.192 | changed so flicker bombing isnt as effective and ACs are more able to respond to changes in bombing | How much efficiency to regain per day. Gain applied hourly.
...
NDefines.NNavy.BASE_GUN_COOLDOWNS = { -- number of hours for a gun to be ready after shooting
	...
	NDefines.NNavy.POSITIONING_PENALTY_FOR_SHIPS_JOINED_COMBAT_AFTER_IT_STARTS = 0 -- each ship that joins the combat will have this penalty to be added into positioning, vanilla is 0.05, changed to improve smaller fleets.
...
-- Spotting defines
NDefines.NNavy.DETECTION_CHANCE_MULT_BASE = 0.1 -- 0.1 base multiplier value for detection chance. Later the chance is an average between our detection and enemy visibility, mult by surface/sub detection chance in the following defines. 
...
NDefines.NNavy.MAX_POSITIONING_PENALTY_FROM_HIGHER_SHIP_RATIO = 1.00  		-- maximum penalty to get from larger fleets
NDefines.NNavy.HIGHER_SHIP_RATIO_POSITIONING_PENALTY_FACTOR = 0.80 			-- (0.25 -> 0.45) this basically means that if the enemy fleet is 45% the size of your fleet you take maximum positioning penalty from fleet size, about -25% attack, -25% screening, -35% aa. I would avoid increasing the maximum too much since it might to lead to some absurd results
...
-- NDefines.NNavy.NAVAL_COMBAT_AIR_STRENGTH_TARGET_SCORE = 20
...
NDefines.NNavy.NAVAL_STRIKE_CARRIER_MULTIPLIER = 18.0              -- damage bonus when planes are in naval combat where their carrier is present (and can thus sortie faster and more effectively)
...
NDefines.NNavy.SHIP_TO_FLEET_ANTI_AIR_RATIO = 0.1 -- (0.2 -> 0.01) -- SilentLegion#1356, MTG 
NDefines.NNavy.ANTI_AIR_POW_ON_INCOMING_AIR_DAMAGE = 0.5 -- (0.2 -> 0.75) -- SilentLegion#1356, MTG 
...
NDefines.NNavy.NAVAL_COMBAT_AIR_SUB_DETECTION_MAX = 5.0
NDefines.NNavy.NAVAL_COMBAT_AIR_SUB_DETECTION_SLOPE = 5.0						-- lower means sharper curve (ramps up very fast, then flatten out very fast). Must be >0
...
NDefines.NNavy.COMBAT_DETECTED_CONVOYS_FROM_SURFACE_DETECTION_STAT = 0.15		-- Each 1.0 of surface_detection that ship has (equipment stat), gives x% of convoys discovered from total travelling along the route
...
NDefines.NNavy.CONVOY_ATTACK_BASE_FACTOR = 0.2                            	-- base % of convoys that get interceptedy
...
NDefines.NNavy.CONVOY_HIT_PROFILE = 150.0  									-- convoys has this contant hit profile
...
NDefines.NAI.MINIMUM_EQUIPMENT_TO_ASK_LEND_LEASE = -1999998	-- WAS 100, changed to prevent exploiting AI for ~150k guns at the start of WW2 | AI will accept to lend lease this equipment only if our stockpile is less than that.
